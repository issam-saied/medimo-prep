...
const submitForm = async () => {
    ...
    try {
        await api.put(`/api/medications/${route.params.id}`, {
            name: form.name,
            form: form.form,
...
onMounted(async () => {
    try {
        const response = await api.get(`/api/medications/${route.params.id}`)
        const medication = response.data.data
        form.name = medication.name
        form.form = medication.form
        form.strength = medication.strength
        form.unit = medication.unit
    } catch (error) {
        errorMessage.value = 'Failed to load medication data.'
    } finally {
        loading.value = false
    }
})
